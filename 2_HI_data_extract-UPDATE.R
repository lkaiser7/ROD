...
rod<-read.csv(paste0(outDir, "ALL_ROD.csv"), header = T)
head(rod)
...
# # ONLY NEED TO RUN ONCE FOR PROCESSING - FILE DOES NOT UPDATE (HAWAII ISLAND ONLY)
# # load elevation data
# hi_elev<-raster(paste0(envDir, "hawaii_elev/hawaii.bil"), native = T) # feet
# # set specific utm projection (NAD83, Zone 5)
# utmSys5<-'+proj=utm +zone=5 +datum=NAD83 +units=m +no_defs +ellps=GRS80 +towgs84=0,0,0'
# # set projection
# crs(hi_elev)<-utmSys5
# # reduce file size
# hi_elev<-aggregate(hi_elev, fact = 30, fun = modal)
# # reproject raster to Lat/Lon coordinates
# hi_elev<-projectRaster(hi_elev, crs = LatLon, method = "ngb")
# # extract data
# hi_elev_spdf<-as.data.frame(as(hi_elev, "SpatialPixelsDataFrame"))
# # removing background 0 values
# hi_elev_spdf$hawaii[which(hi_elev_spdf$hawaii == 0)]<-NA
# # remove missing values
# hi_elev_spdf<-hi_elev_spdf[complete.cases(hi_elev_spdf$hawaii),]
# # save outputs
# writeRaster(hi_elev, format = "GTiff", overwrite = T,
#             paste0(envDir, "hawaii_elev/hawaii_elev.tif"))
# write.csv(hi_elev_spdf, paste0(outDir, "hi_elev_spdf.csv"))

#----- OHIA DISTRIBUTION -----#

# load ohia distribution raster
ohia_rast<-raster(paste0(envDir, "higap/HIGAP_ohia_forest_500m.tif"))
# remove background values
ohia_rast[ohia_rast == 0]<-NA
...
rod_ohia<-extract(hi_ohia, cbind(rod$X_LON, rod$Y_LAT))
...
all_veg_spdf$COLOR<-veg_class$COLOR[match(all_veg_spdf$HIGAP, veg_class$Value)]
...
# match color to classification value
ohia_vars$HIGAP_COLOR<-veg_class$COLOR[match(ohia_veg, veg_class$Value)]

# extract vegetation classes from Hawaii Island at rod points
rod_veg<-extract(hi_veg, cbind(rod$X_LON, rod$Y_LAT))
...
# match color to classification value
all_br_spdf$COLOR<-bioregs$COLOR[match(all_br_spdf$bioregions, bioregs$FID)]
...
# match color to classification value
ohia_vars$BIOREG_COLOR<-bioregs$COLOR[match(ohia_br, bioregs$FID)]
...
all_geol_spdf$AGE_COLOR<-age_table$COLOR[match(all_geol_spdf$AGE_GROUP, age_table$age_group)]
all_geol_spdf$name<-name_table$NAME[match(all_geol_spdf$NAME, name_table$name)]
all_geol_spdf$NAME_COLOR<-name_table$COLOR[match(all_geol_spdf$NAME, name_table$name)]
...
# match color to classification value
ohia_vars$AGE_COLOR<-age_table$COLOR[match(ohia_geol$AGE_GROUP, age_table$age_group)]
# add desired geology columns to master ohia data set
ohia_vars$name<-name_table$name[match(ohia_geol$NAME, name_table$NAME)]
ohia_vars$NAME<-ohia_geol$NAME
# match color to classification value
ohia_vars$NAME_COLOR<-name_table$COLOR[match(ohia_geol$NAME, name_table$NAME)]
...
# extract fence values from Hawaii across ohia distribution
ohia_fences<-extract(fence_stack$GlobalID, cbind(ohia_vars$x, ohia_vars$y))
# add fence columns to master rod data set
ohia_vars$fenced<-ohia_fences
ohia_vars$FENCED<-ohia_fences
ohia_vars$FENCED[which(ohia_vars$FENCED > 0)]<-"Fenced"
ohia_vars$FENCED[which(is.na(ohia_vars$FENCED))]<-"Not Fenced"
